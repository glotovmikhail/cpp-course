...
const int64_t main_base = (1 << 30);
int64_t base = main_base;
const int64_t digits = 9;
...
big_integer::big_integer(int a) {
    int64_t b = static_cast<int64_t> (a);
    sign = 1;
    if (b < 0) {
        sign = -1;
        b = -b;
...
big_integer::big_integer(std::string const& str) {
    ...
    numbers.clear();
    int64_t pos = 0;
    while (pos < (int64_t) str.size() && (str[pos] == '-' || str[pos] == '+')) {
        if (str[pos] == '-') sign = -sign;
        pos++;
    }
    for (int64_t i = (int64_t) str.size() - 1; i >= pos; i -= digits) {
        int64_t x = 0;
		for (int64_t j = max(pos, i - digits + 1); j <= i; j++) {
			x = x * 10 + static_cast<int64_t>(str[j] - (char) '0');
		}
        numbers.push_back(x);
    }
    clear_zeroes();
    base = 1000 * 1000 * 1000;
    int64_t new_base = main_base;
    big_integer res;
    res.sign = sign;
    *this = abs();
    while (*this > 0) {
...
big_integer& big_integer::operator+=(big_integer const& rhs) {
    if (sign == rhs.sign) {
        big_integer res = rhs;
        for (int64_t i = 0, carry = 0; i < (int64_t) max(numbers.size(), rhs.numbers.size()) || carry; i++) {
...
big_integer& big_integer::operator-=(big_integer const& rhs) {
    if (sign == rhs.sign) {
        if (abs() >= rhs.abs()) {
            big_integer res = *this;
            for (int64_t i = 0, carry = 0; i < rhs.numbers.size() || carry; i++) {
...
big_integer& big_integer::operator*=(int64_t v) {
    if (v < 0) {
        sign = -sign;
        v = -v;
    }
    for (int64_t i = 0, carry = 0; i < numbers.size() || carry; ++i) {
		if (i == numbers.size()) {
			numbers.push_back(0);
		}
        int64_t cur = numbers[i] * (int64_t) v + carry;
        carry = (int64_t) (cur / base);
        numbers[i] = (int64_t) (cur % base);
    }
    clear_zeroes();
    return *this;
}
...
big_integer divide(const big_integer &a1, const big_integer &b1) {
    ...
    int64_t n = base / (b1.numbers.back() + 1);
    big_integer a = a1.abs() * n;
    big_integer b = b1.abs() * n;
    big_integer ans, r;
    ans.numbers.resize(a.numbers.size());

    for (int64_t i = a.numbers.size() - 1; i >= 0; i--) {
        r *= base;
        r += a.numbers[i];
        int64_t s1 = r.numbers.size() <= b.numbers.size() ? 0 : r.numbers[b.numbers.size()];
        int64_t s2 = r.numbers.size() <= b.numbers.size() - 1 ? 0 : r.numbers[b.numbers.size() - 1];
        int64_t d = ((int64_t) base * s1 + s2) / b.numbers.back();
        r -= b * d;
        while (r < 0) {
            r += b;
            d--;
...
big_integer& big_integer::operator/=(int64_t x) {
	int64_t v = x;
    if (v < 0) {
        sign = -sign;
        v = -v;
    }
    for (int64_t i = numbers.size() - 1, rem = 0; i >= 0; i--) {
        int64_t cur = numbers[i] + rem * (int64_t) base;
        numbers[i] = (int64_t) (cur / v);
        rem = (int64_t) (cur % v);
    }
    clear_zeroes();
    return *this;
...
int big_integer::operator%(int64_t v) {
    if (v < 0)
        v = -v;
    int64_t m = 0;
...
std::ostream& operator<<(ostream &stream, const big_integer &v) {
	...
	if (buf.numbers.empty()) {
		stream << 0;
	}
	else {
		stream << buf.numbers.back();
	}
    for (int64_t i = buf.numbers.size() - 2; i >= 0; i--)
...
void big_integer::convert_to_addcode(vector<int64_t>& a) {
...
void big_integer::convert_to_normal(vector<int64_t>& a) {
    if (a.back() == 0) {
        numbers = a;
        numbers.pop_back();
    } else {
...
{
    if (*this < 0) (*this)++;
    for (int64_t i = 0; i < rhs; i++) *this *= 2;
    if (*this < 0) (*this)--;
    return *this;
}
